...
class TestReadWriteRuns:
    ...
    def test_roundtrip(self, log_path):
        ...
        assert _read_runs(log_path) == runs

    def test_creates_parent_dirs(self, tmp_path):
        nested = tmp_path / "a" / "b" / "runs.json"
        _write_runs(nested, [RunEntry(run=1)])
        assert _read_runs(nested) == [RunEntry(run=1)]


class TestGetNextRunNumber:
    def test_empty_list(self):
        assert _get_next_run_number([]) == 1

    def test_sequential(self):
        assert _get_next_run_number([RunEntry(run=1), RunEntry(run=2)]) == 3

    def test_gaps(self):
        assert _get_next_run_number([RunEntry(run=1), RunEntry(run=5)]) == 6


class TestUpdateRunEntry:
    def test_updates_scalar_field(self, log_path):
        _write_runs(log_path, [RunEntry(run=1, status="running")])
        _update_run_entry(log_path, 1, {"status": "completed"})
        runs = _read_runs(log_path)
        assert runs[0].status == "completed"

    def test_merges_dict_field(self, log_path):
        _write_runs(log_path, [RunEntry(run=1, performance=RunPerformance(start_time="t0"))])
        _update_run_entry(log_path, 1, {"performance": {"phase1_time_s": 100}})
        runs = _read_runs(log_path)
    ...
    def test_noop_for_missing_run(self, log_path):
        _write_runs(log_path, [RunEntry(run=1, status="running")])
        _update_run_entry(log_path, 99, {"status": "failed"})
        runs = _read_runs(log_path)
